// Copyright 2023 The Go Authors and Anfid. All rights reserved.
...
//
// This package is a fork of golang.org/x/crypto/argon2. Changes introduced
// only affect package's public API. Full Argon2 functionality is exposed for
// the sake of completeness.
//
// For a detailed specification of Argon2 see
// https://github.com/P-H-C/phc-winner-argon2/blob/master/argon2-specs.pdf.
//
// If you aren't sure which function you need, use Argon2id and
// the parameter recommendations for your scenario. All parameter
...
// DeriveKey derives a key from the password, salt, secret, data and cost
// parameters using selected argon2 flavor returning a byte slice of length
// keyLen that can be used as cryptographic key. The CPU cost and parallelism
...
//
// The draft RFC recommends time=1, and memory=64*1024 is a sensible number.
// If using that amount of memory (64 MB) is not possible in some contexts then
// the time parameter can be increased to compensate.
//
// The flavor parameter specifies the flavor that will be used to derive the
// key. secret is optional and can be used as a constant extra parameter used
// for hashing that won't be stored in the database. data is also optional, it
// represents arbitrary extra data. The time parameter specifies the number of
// passes over the memory and the memory parameter specifies the size of the
// memory in KiB. See recommendations in documentation for each flavor if
// you're not sure what to use. The number of threads can be adjusted to the
// numbers of available CPUs. Remember to get a good random salt.
func DeriveKey(flavor int, password, salt, secret, data []byte, time, memory uint32, threads uint8, keyLen uint32) []byte {
	if flavor != Argon2d && flavor != Argon2i && flavor != Argon2id {
		panic("argon2: Unknown argon2 variant used")
	}
	if time < 1 {
	...
	h0 := initHash(password, salt, secret, data, time, memory, uint32(threads), keyLen, flavor)
	...
	processBlocks(B, time, memory, uint32(threads), flavor)
